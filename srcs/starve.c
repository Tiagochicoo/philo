...
/*   Updated: 2022/09/15 17:29:02 by tpereira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_death_mealz(t_info *info)
{
	if (info->num == 1)
	{
	...
	{
		printf("info->philo_died [%d]\n", info->philo_died);
		//printf("time to die usleep(%ld)\n", elapsed_time(philo) - info->time_to_die);
		//usleep(elapsed_time(info->philos[info->philo_died - 1]) - info->time_to_die);
		print_msg("...has died", info->philos[0], RED);
	}
	stop_meal(info);
}
